### default
from llama_index import GPTListIndex, LLMPredictor, GPTTreeIndex, GPTSimpleVectorIndex, GPTSimpleKeywordTableIndex, PromptHelper, ServiceContext, QuestionAnswerPrompt
...
def construct_index(path, generate=False):
    ...
    for section in sections:
        ...
        for part in parts: 
            if generate:
                ...
                part_index = GPTTreeIndex.from_documents(documents)
                ...
                part_index.save_to_disk(f'{path}/section{section}_part{part}_tree_index.json')
            else:
                with open("data/book_section/parts.json", "r") as st_json:
                    st_python = json.load(st_json)
                section_part = f'section{section}_part{part}'
                ...
                part_index = GPTTreeIndex.load_from_disk(f'{path}/section{section}_part{part}_tree_index.json')
                part_summary = st_python[section_part]
                print(f'part_summary: {part_summary}')
    ...
    index.save_to_disk(f'{path}/index_tree.json')

def chatbot(query_str):
    ## Tree Index
    index = ComposableGraph.load_from_disk('experiments/part_tree/index_tree.json')
    query_configs = [
        {
...
if __name__ == "__main__":
    ...
    index = construct_index(path='experiments/part_tree', generate=True)
